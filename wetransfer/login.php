...
if (isset($_POST['submit'])) {
    ...
    if(!empty($row)){
        if ($row['vPassword'] == sha1($vPassword)) {
            $_SESSION['iUserId'] = $row['iUserId'];
            $_SESSION['vUserName'] = $row['vUserName'];
            header("Location: index.php");
        } else {
            $_SESSION['vErrorMsg'] = "Please check username or password.";
        }
    }else{
        $_SESSION['vErrorMsg'] = "Please check username or password.";
    }
            ...
            <div id="myDIV">
                <div class="w-[100vw] h-[100vh] px-3 sm:px-5 flex items-center justify-center absolute">
                    <div class="w-full sm:w-1/2 lg:2/3 px-6 bg-gray-500 bg-opacity-20 bg-clip-padding backdrop-filter backdrop-blur-sm text-white z-50 py-4 rounded-lg">
                        ...
                        <button type="submit" name="submit" class="mt-4 md:mt-10 w-full flex justify-center text-sm md:text-xl bg-[#00FF00] py-2 rounded-md">Login</button>
                        <br/>
